...
export default function TravelGrantAssistant() {
  ...
  const [currentStep, setCurrentStep] = useState<"question" | "search" | "compare" | "plan" | "complete">("question")
  ...
  const [sidePanel, setSidePanel] = useState<{ isOpen: boolean; url: string; title: string }>({
    isOpen: false,
    url: "",
    title: "",
  })
  ...
  const [showDetailFor, setShowDetailFor] = useState<{ dayIndex: number; activityIndex: number } | null>(null)
  const [showMemoFor, setShowMemoFor] = useState<{ dayIndex: number; activityIndex: number } | null>(null)
  const [queryHistory, setQueryHistory] = useState<string[]>([])
  const [changedItems, setChangedItems] = useState<Set<string>>(new Set())
  const [sortBy, setSortBy] = useState<"relevance" | "deadline" | "latest">("relevance")
  ...
  const { messages, input, handleInputChange, handleSubmit, isLoading, setMessages, setInput } = useChat({
    ...
    onFinish: (message) => {
      ...
      if (currentStep === "complete" && message.content) {
        ...
        if (
          content.includes("제목") &&
          (content.includes("넣어") || content.includes("바꿔") || content.includes("변경"))
        ) {
          const titleMatch = message.content.match(/제목[은는]?\s*['"]([^'"]+)['"]/)
          if (titleMatch) {
        ...
        if (
          content.includes("요약") &&
          (content.includes("넣어") || content.includes("써") || content.includes("작성"))
        ) {
          const summaryMatch = message.content.match(/요약[에는]?\s*['"]([^'"]+)['"]/)
          if (summaryMatch) {
      ...
      if (message.toolInvocations) {
        message.toolInvocations.forEach((invocation: any) => {
          if (invocation.toolName === "analyzeConditions" && invocation.result) {
          ...
          activities: [
            {
              name: "왕궁리 유적 탐방",
              location: "익산시 왕궁면",
              comment: "백제 왕궁터로 추정되는 역사적 장소, 넓은 잔디밭에서 휴식 가능",
              rating: 4.5,
              likes: 445,
            ...
            },
            {
              name: "가족 카페 방문",
              location: "익산시 부송동",
              comment: "키즈존이 있는 대형 카페, 부모는 휴식하고 아이는 놀 수 있어요",
              rating: 4.3,
              likes: 189,
  ...
  const handleActivitySelect = (dayIndex: number, activityIndex: number) => {
    ...
    newPlan[dayIndex].activities[activityIndex].selected = !newPlan[dayIndex].activities[activityIndex].selected
    setTravelPlan(newPlan)
  }

  const handleMemoChange = (dayIndex: number, activityIndex: number, memo: string) => {
    const newPlan = [...travelPlan]
  ...
  const saveToHistory = () => {
    const newHistoryItem: HistoryItem = {
      ...
      title: `${programs.find((p) => selectedPrograms.includes(p.id))?.name || "여행 계획"} - ${new Date().toLocaleDateString()}`,
      step: currentStep,
      programs,
      selectedPrograms,
      travelPlan,
  ...
  const goToStep = (step: "question" | "search" | "compare" | "plan" | "complete") => {
    setCurrentStep(step)
  }
  ...
  const openOnlineDocument = (style: "simple" | "standard" | "visual" | "calendar" | "calendar2" = "standard") => {
    // 심플버전 (기존 유지)
      ...
      ${day.activities
        ...
        <div class="activity">
          ...
          <div class="activity-content">
            ...
            <div class="activity-details">${activity.location} • ${activity.duration} • ${activity.category}</div>
            <div class="activity-description">${activity.comment}</div>
            ${activity.userMemo ? `<div class="memo">${activity.userMemo}</div>` : ""}
          </div>
        </div>
      `,
        )
        ...
        <div class="activities-list">
          ${day.activities
            ...
            <div class="activity-item">
              <div class="activity-time">${index === 0 ? "오전" : index === day.activities.length - 1 ? "저녁" : index < day.activities.length / 2 ? "오전" : "오후"}</div>
              <div class="activity-content">
                <div class="activity-name">
                  ${activity.name}
                  <span class="category-badge category-${activity.category}">${activity.category}</span>
                </div>
                <div class="activity-location">📍 ${activity.location} • ⏰ ${activity.duration}</div>
                <div class="activity-description">${activity.comment}</div>
                ${activity.userMemo ? `<div class="activity-memo">💭 ${activity.userMemo}</div>` : ""}
              </div>
            </div>
          `,
            )
...
<body>
  ...
  <div class="calendar-grid">
    ...
    ${Array.from({ length: 42 }, (_, i) => {
      ...
      const displayNum = isCurrentMonth ? dayNum : dayNum <= 0 ? 31 + dayNum : dayNum - 30
        ...
        travelPlan.forEach((day, index) => {
          ...
          if (dayDate === dayNum) {
            ...
            if (day.activities.length > 3) {
              eventsHtml += `<div class="event-item">+${day.activities.length - 3}개 더</div>`
            }
          }
        })
      }
      ...
      return `
        <div class="calendar-day ${hasEvents ? "has-events" : ""}" ${hasEvents ? `onclick="showDayDetail(${dayIndex})"` : ""}>
          <div class="day-number ${!isCurrentMonth ? "other-month" : ""}">${displayNum}</div>
          ${eventsHtml}
        </div>
      `
    }).join("")}
...
`
    ...
    const getPlaceholderImage = (activity: any) => {
      ...
      return `/placeholder.svg?height=60&width=80&query=${encodeURIComponent(queries[activity.name as keyof typeof queries] || activity.name)}`
    }
          ...
          <div class="activity-item">
            ...
            <div class="activity-card">
              <div class="activity-header">
                ${
                  style === "visual"
                    ? `<img src="${getPlaceholderImage(activity)}" alt="${activity.name}" class="activity-image" />`
                    : ""
                }
                <div class="activity-content">
...
`
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-indigo-50 text-gray-900 flex">
      ...
      {/* Main Content */}
      <div className={`flex-1 transition-all duration-300 ease-in-out ${showHistory ? "ml-80" : "ml-0"}`}>
        <div className="p-6">
          <div className="max-w-5xl mx-auto">
            ...
            {(currentStep === "question" || currentStep === "search" || currentStep === "compare") && (
              <QuestionSection
                currentStep={currentStep}
            ...
            {sidePanel.isOpen && (
              <div className="fixed inset-0 z-50 flex">
                <div
                  ...
                  onClick={() => setSidePanel({ isOpen: false, url: "", title: "" })}
                ></div>

                <div className="fixed inset-y-0 right-0 w-1/2 bg-white shadow-xl z-50 border-l border-gray-200">
                  <div className="flex items-center justify-between p-4 border-b border-gray-200">
                    <h3 className="font-medium text-gray-900">{sidePanel.title}</h3>
                    <Button variant="ghost" onClick={() => setSidePanel({ isOpen: false, url: "", title: "" })}>
                      <X className="w-4 h-4" />
                    </Button>
                  </div>
                  <iframe src={sidePanel.url} className="w-full h-full" title={sidePanel.title} />
                </div>
              </div>
            )}
