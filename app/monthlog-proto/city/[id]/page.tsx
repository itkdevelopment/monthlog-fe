...
export default function CityDetailPage() {
  ...
  const cityData = {
    ...
    description:
      '한국의 대표적인 휴양지이자 ���지털 노마드들에게 인기 있는 워케이션 명소입니다.',
                    ...
                    <div key={index} className="flex items-center space-x-2">
                      ...
                      <div className="flex-1 bg-gray-200 rounded-full h-4">
                        <div
                          ...
                          style={{
                            width: `${
                              (count / Math.max(...data.distribution)) * 100
                            }%`,
                          }}
                        ></div>
                      </div>
                ...
                <div className="space-y-2">
                  {data.priceDistribution.map(
                    (count: number, index: number) => {
                      const percentage =
                        (count / Math.max(...data.priceDistribution)) * 100;
                      return (
                        <div
                          key={index}
                          ...
                          <div
                            ...
                            style={{
                              width: `${
                                (item.avgCost /
                                  Math.max(
                                    ...data.items.map((i: any) => i.avgCost),
                                  )) *
                                100
                              }%`,
                            }}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {data.categories.map((category: string, index: number) => {
                      const count = data.places.filter(
                        (place: any) => place.category === category,
                      ).length;
                      return (
                        <div
                          key={index}
              ...
              <div>
                ...
                <div className="space-y-2">
                  {data.items.map((item: any, index: number) => {
                    const percentage =
                      (item.votes /
                        Math.max(...data.items.map((i: any) => i.votes))) *
                      100;
                    return (
                      <div key={index} className="flex items-center space-x-3">
                        <span className="w-20 text-sm text-gray-900 font-medium">
                          {item.name}
                        </span>
                        <div className="flex-1 bg-gray-200 rounded-full h-3">
                          <div
    ...
    detailItems: [
      {
        ...
        label: '인터넷 평균 속도',
        value: '100Mbps',
        modalKey: 'internetSpeed',
      },
      {
        icon: Zap,
        label: '전력 안정성',
        value: '정전 거의 없음',
        modalKey: 'powerStability',
      },
      {
        icon: Wifi,
        label: '무료 Wi-Fi 접근성',
        value: '카페, 공공장소에서 매우 용이',
        modalKey: 'wifiAccess',
      },
      {
        icon: Coffee,
        label: '코워킹 스페이스',
        value: '5곳 이상 / 1일 평균 2만원',
        modalKey: 'coworkingSpaces',
      },
      {
        icon: Coffee,
        label: '노트북 하기 좋은 카페',
        value: '사용자 추천 장소 2~3곳',
        modalKey: 'cafes',
      },
      {
        icon: Calendar,
        label: '단기 멤버십 정보',
        value: '주(Week)단위 이용권 구매 용이',
        modalKey: 'membership',
      },
    ],
    ...
    detailItems: [
      {
        ...
        label: '물가 만족도',
        value: '7/10',
        modalKey: 'satisfaction',
      },
      {
        icon: Home,
        label: '월세 (원룸/스튜디오 기준)',
        value: costData.rentMedian,
        modalKey: 'rent',
      },
      {
        icon: Zap,
        ...
        <div className="mx-6 lg:mx-8 pb-8">
          ...
          <div className="space-y-8">
            ...
            <div ref={workationRef}>
              <WorkationCard
                cardData={workationEnvironmentCardData}
                onExpGain={handleExpGain}
              />
            </div>
