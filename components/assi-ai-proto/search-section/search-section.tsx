...
import { Card, CardContent, CardHeader, CardTitle } from "@/components/assi-ai-proto/ui/card"
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-lg mb-8 p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-2xl font-semibold text-gray-900">추천 프로그램 ({programs.length}개)</h2>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as "relevance" | "deadline" | "latest")}
      ...
      <p className="text-gray-600 mb-8">카드를 눌러 확인하고 관심 있는 항목만 선택해주세요.</p>
        ...
        {programs.map((program) => (
          <Card
            ...
            className={`cursor-pointer transition-all bg-white border-gray-200 hover:border-gray-300 hover:shadow-md ${
              selectedPrograms.includes(program.id) ? "ring-2 ring-[#0000f5] border-[#0000f5] bg-blue-50" : ""
            } ${changedItems.has(program.id) ? "ring-2 ring-green-400 border-green-400 bg-green-50" : ""}`}
            onClick={() => handleProgramSelect(program.id)}
          >
            <CardHeader className="pb-3 border-b border-gray-100">
              <div className="flex items-start justify-between">
                <CardTitle className="text-lg text-gray-900">{program.name}</CardTitle>
                <div className="flex items-center gap-2">
                  ...
                  {sortBy === "deadline" && (
                    <span className="text-xs text-blue-600 font-medium">D-{Math.floor(Math.random() * 50) + 10}</span>
                  )}
                  <div
                    className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-all ${
                      selectedPrograms.includes(program.id) ? "bg-[#0000f5] border-[#0000f5]" : "border-gray-300"
                    }`}
                  >
            ...
            <CardContent className="pt-4">
              <div className="space-y-3">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-700">{program.familyFriendly ? "가족 동반 가능" : "개인 여행"}</span>
                </div>
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation()
                      setSelectedProgramDetail(selectedProgramDetail === program.id ? null : program.id)
                    }}
                  >
                ...
                {selectedProgramDetail === program.id && (
                  <div className="mt-4 p-3 bg-gray-50 rounded-lg text-sm border border-gray-200">
                    <div className="space-y-3">
                      <div>
                        <span className="font-medium text-gray-700">신청 방법:</span>{" "}
                        <span className="text-gray-600">{program.applicationMethod}</span>
                      </div>
                      <div>
                      ...
                      <div>
                        <span className="font-medium text-gray-700">여행계획서:</span>{" "}
                        <span className="text-gray-600">{program.needsPlan ? "필요" : "불필요"}</span>
                      </div>
